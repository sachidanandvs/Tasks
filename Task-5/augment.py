...
def gen_eda(train_orig, output_file, alpha_sr, alpha_ri, alpha_rs, alpha_rd, num_aug=9):
    ...
    train_data = pd.read_csv("/home/sachi/Desktop/intern/yelp_review_full_csv/train.csv",header = None)
    train_data_augmented = pd.DataFrame(columns = train_data.columns)

    for i in range(6500):
        label = train_data[0][i]
        sentence = train_data[1][i]
        if len(list(map(len, sentence.split()))) == 1:
            continue

        aug_sentences = eda(sentence, alpha_sr=alpha_sr, alpha_ri=alpha_ri, alpha_rs=alpha_rs, p_rd=alpha_rd, num_aug=num_aug)
        for i,aug_sentence in enumerate(aug_sentences,0):
            train_data_augmented = train_data_augmented.append({0: label, 1: aug_sentence}, ignore_index=True)


    print("generated augmented sentences with eda for " + train_orig + " to " + output_file + " with num_aug=" + str(num_aug))
    train_data_augmented.to_csv(output_file+".csv")
